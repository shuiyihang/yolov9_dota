...
class DualDDetect(nn.Module):
    ...
    def forward(self, x):
        ...
        box, cls, angle = torch.cat([di.view(shape[0], self.no, -1) for di in d1], 2).split((self.reg_max * 4, self.nc, self.angle_dim), 1)
        dbox = dist2bbox(self.dfl(box), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides
        box2, cls2, angle2 = torch.cat([di.view(shape[0], self.no, -1) for di in d2], 2).split((self.reg_max * 4, self.nc, self.angle_dim), 1)
        dbox2 = dist2bbox(self.dfl2(box2), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides
        y = [torch.cat((dbox, cls.sigmoid(),angle), 1), torch.cat((dbox2, cls2.sigmoid(),angle2), 1)]
        return y if self.export else (y, [d1, d2])
